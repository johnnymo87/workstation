...
| `dd_pat` | Datadog CLI (dd-cli) + MCP — preferred Personal Access Token (Bearer) | Exported as `DD_PAT` in bash |
| `dd_api_key` | Datadog CLI (dd-cli) — deprecated app+api fallback | Exported as `DD_API_KEY` in bash |
| `dd_app_key` | Datadog CLI (dd-cli) — deprecated app+api fallback | Exported as `DD_APP_KEY` in bash |
