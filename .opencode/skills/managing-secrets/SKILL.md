...
| `typesafe_api_key` | TypeSafe AI (jev decision model) via `typesafe-sdk`. **devbox only** so far; cloudbox to follow in its own `secrets/cloudbox.yaml` (devbox cannot encrypt to that file). | Exported as `TYPESAFE_API_KEY` in bash and by `shell-env.ts` |
