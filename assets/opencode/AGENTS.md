...
| [swarm-messaging](skills/swarm-messaging/SKILL.md) | cross | **Message economy first** (fewer messages, not shorter; no acks; batch and hold; never restate a swarm message to a human who can see it), **the coordinator role is banned** (it doubles message count; use a durable artifact + direct worker-to-human reporting instead), the relay hazards that remain (retract your own relays, don't propose from a stale board, silence reads as endorsement), then the protocol: `swarm_send`/`swarm_read`/`swarm_list`, the `<swarm_message>` envelope, kinds, priority, threading, replay. |
...
| [swarm-shaped-work](skills/swarm-shaped-work/SKILL.md) | cross | When to swarm vs. iterate sequentially. Flat, coordinator-free worker topology with shared state in a plan file. Spin-up sequence (write the plan → `opencode-launch` × N → one roster broadcast → kick off). |
...
| [tagging-sessions](skills/tagging-sessions/SKILL.md) | cross | `oc-tags` spend attribution: tags name programs of work (not worktrees, systems, or ticket keys), one tag per tracker epic, don't guess a tag, snapshot before bulk retag, spend is a coverage detector not a ranking. The org tag list is in its Confluence-fetched `INTERNAL.md` (work hosts). |
...
| [formatting-slack-messages](skills/formatting-slack-messages/SKILL.md) | cross | Composing a Slack post — the MCP sends under the human's own name, so disclose authorship, get the first draft approved, reply in-thread, and link everything a reader cannot otherwise open. Plus mrkdwn dialect quirks (single-asterisk bold, underscore italic, no headers, angle-bracket links) and attaching files by `file_path` from the staging dir, never base64 through the context window. |
