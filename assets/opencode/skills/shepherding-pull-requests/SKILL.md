...
**The handle named above is the one for this host.** It is rendered per-host at deploy time (`users/dev/opencode-skills.nix`), so the file you are reading names the reviewer this machine actually defaults to. Do not substitute the other twin because you prefer it.
