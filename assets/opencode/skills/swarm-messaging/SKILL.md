...
description: Use when sending messages to other opencode sessions on the same machine (swarm coordination), or when you receive a <swarm_message> envelope as the text of a user-message turn. Also use when ending a turn on a future checkpoint — "check after the 09:00 run", "revisit once CI finishes", "verify the deploy tomorrow" — because you must schedule a wake with swarm_schedule before you stop, or nothing will remind you.
---
...
`swarm_schedule` delivers a message at a future time. The dominant use is
waking *yourself*: `to` defaults to the calling session.

The delivery is durable, not a timer. The row lives in the daemon's SQLite and
is polled, so it survives daemon restarts, the nightly 03:00 reset, and reboots.
This is measured, not hoped for: a wake scheduled at 21:30 was delivered at
09:00 the next morning, 119ms late, by a daemon process that had been restarted
at 03:00 — a different process than the one that accepted it.

**The imperative: if you are ending a turn on a future checkpoint, schedule the
wake before you stop.** "I'll check the prod cron after the 09:00 run" is not a
plan unless a wake exists. Nothing else will remind you.
...
- **Expiry defaults to 6h** after the delivery time. Override with
  `expires_in`. A wake that cannot be delivered within its window expires
  rather than arriving at an arbitrary later time.
...
The floor only catches `"check on it"`. Length is not the rule — a reader with
no memory of you is.
...
Everything in *Message economy* applies, and more sharply — a wake interrupts a
future session that has no context for it. Schedule one when a specific action
is genuinely blocked on the passage of time: a deploy to verify, a nightly job
to check, a rate limit to clear. Not to "check in", not to review your own
progress, and never on a recurring cadence. A wake you would not bother to
write a paragraph for is a wake nobody needed.

If the checkpoint gets handled early, cancel the wake —
...
already done is precisely the pointless interruption this section is about.
