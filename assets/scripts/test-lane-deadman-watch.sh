...
cat > "$CADENCE_CMD" <<EOF
#!$BASH
# Args arrive as: --user is-enabled <unit>
state="\$(cat "$CADENCE_STATE_FILE" 2>/dev/null)"
case "\$state" in
...
EOF
chmod +x "$CADENCE_CMD"
echo enabled > "$CADENCE_STATE_FILE"
...
echo disabled > "$CADENCE_STATE_FILE"
...
echo not-found > "$CADENCE_STATE_FILE"
...
echo silent > "$CADENCE_STATE_FILE"
