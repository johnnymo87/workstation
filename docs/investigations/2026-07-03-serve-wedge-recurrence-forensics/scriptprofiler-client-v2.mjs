...
setTimeout(() => { console.log("GLOBAL TIMEOUT"); process.exit(3); }, 2900000);
let id = 0;
...
function send(method, params = {}) {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => { if (pending.has(msgId)) { pending.delete(msgId); reject(new Error("timeout " + method)); } }, 1200000);
  });
}
...
const samples = result?.samples?.stackTraces || []; (await import("node:fs")).default.writeFileSync("/tmp/opencode/sp3-raw.json", JSON.stringify({scripts:[...scripts.entries()],samples}));
...
for (const st of samples) {
  ...
  if (frames.length) {
    const f = frames[0];
    const key = (f.name || "<anon>") + " @ " + (scripts.get(String(f.sourceID)) || f.sourceID) + ":" + f.line + ":" + (f.column ?? "?");
