...
OUT=/home/dev/s3-sampling/samples.tsv
TS=$(date +%s)

if [ ! -s "$OUT" ]; then
  printf 'ts\tport\tserve_id\tpid\tboot\tmem_bytes\trss_kb\tthreads\tfds\tinotify_fds\tinotify_watches\tnotify_debounce\tassign_total\tassign_active_1h\tassign_active_10m\tanon\tfile\tslab\tpagetables\tswap\tmem_high\tactive_file\n' > "$OUT"
fi
...
for p in 4096 4097 4098 4099; do
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$TS" "$p" "$sid" "$pid" "$boot" "$mem" "$rss" "$thr" "$fds" "$inof" "$inow" "$deb" "$at" "$a1" "$a10" \
