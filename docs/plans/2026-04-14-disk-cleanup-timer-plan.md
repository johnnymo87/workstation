# Disk Cleanup Timer Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a nightly systemd timer on cloudbox that automatically cleans stale worktrees, orphan Bazel output bases, and safe caches.

**Architecture:** A single bash script (`~/.local/bin/disk-cleanup`) deployed via `home.file`, triggered by a systemd user timer defined in `home.cloudbox.nix`. The script auto-discovers repos and cleans only what's safe. All output goes to journal.

**Tech Stack:** Bash, systemd user timers, git, python3 (for Bazel hash mapping), nix-collect-garbage

---

### Task 1: Create the disk-cleanup script

**Files:**
- Create: `users/dev/disk-cleanup.nix` (new module with script + systemd units)
- Modify: `users/dev/home.cloudbox.nix` (import the new module)

**Step 1: Write the disk-cleanup script as a new nix module**

Create `users/dev/disk-cleanup.nix`:
...
**Step 2: Import the new module in home.nix**

In `users/dev/home.nix`, add `./disk-cleanup.nix` to the imports list (after `./home.cloudbox.nix`):
...
```nix
  imports = [
    ...
    ./home.devbox.nix
    ./home.cloudbox.nix
    ./disk-cleanup.nix        # <-- add this line
    ./home.crostini.nix
    ...
  ];
```

**Step 3: Run a dry test on cloudbox**

```bash
# Apply home-manager to deploy the script
nix run home-manager -- switch --flake .#cloudbox

# Verify script is deployed and executable
ls -la ~/.local/bin/disk-cleanup

# Verify timer is registered
systemctl --user list-timers | grep disk-cleanup

# Do a manual test run and watch the output
systemctl --user start disk-cleanup.service
journalctl --user -u disk-cleanup.service -f
```

**Step 4: Verify no company references**

```bash
rg -i "wonder" users/dev/disk-cleanup.nix
rg "cops-[0-9]" -i users/dev/disk-cleanup.nix
...
```bash
git add users/dev/disk-cleanup.nix users/dev/home.nix
git commit -m "Add nightly disk cleanup timer for cloudbox

Automates safe cleanup that previously required manual
intervention: merged/abandoned worktrees, orphan Bazel
output bases, stale caches, nix GC, and OpenCode WAL
checkpointing."
```

---

### Task 2: Update the cleaning-disk skill to reference the automation

**Files:**
- Modify: `assets/opencode/skills/cleaning-disk/SKILL.md`

**Step 1: Add a note about the nightly timer**

Add a section near the top of the skill referencing the automated cleanup, so future sessions know it exists and what it covers vs what still needs manual intervention.

**Step 2: Commit**

```bash
git add assets/opencode/skills/cleaning-disk/SKILL.md
git commit -m "Update cleaning-disk skill to reference nightly timer"
```
