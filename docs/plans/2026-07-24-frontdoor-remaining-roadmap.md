...
4. **HTML-poison guard** (bead `workstation-m3z2`) — **PROMOTED from optional to required** (Correction 3). **BUILT + DEPLOYED 2026-07-25.** Any `text/html` on a forwarded response → JSON 502 (widened from `session-path`; `GET /api/fs/read/*` exempted, since it serves runtime-typed raw bytes). Load-bearing because skew is the *designed steady state* here (~27 pin bumps in 6 weeks; `pull-workstation` every 4h; pool deliberately not bounced).
   **CORRECTION — this item does NOT deliver "a clean error instead of a frozen TUI".** That claim was false and is retracted. Verified in the deployed patch source (`opencode-patched/patches/tui-door-attach.patch:192-204`): `reconcilePending` throws on **any** `.error`, so a JSON 502 throws exactly like an HTML parse blowup → the same infinite reconnect → the same frozen TUI. The guard converts every HTML case *into* the non-HTML residual it was said to leave behind. What it actually buys: diagnosability, correct behavior for non-TUI clients, and collapsing two failure shapes into one (which makes Part B *simpler*). Only **Part B** un-freezes the TUI — see `workstation-fdb1`. Detail: `docs/plans/2026-07-25-m3z2-html-poison-guard.md` §2.
   It therefore does **not** by itself make the `a0zj` "mitigated, accepted" story honest; `a0zj`'s close-out disposition must be re-argued on the narrower benefit above.
...
- **Part B reconcile hardening** (old item 2) — ~~the HTML-poison guard covers the same user-visible harm at the door for less~~ **THIS DEMOTION RATIONALE IS RETRACTED (2026-07-25).** The guard does NOT cover the same harm: the bricking condition is `reconcilePending` throwing on ANY error, which a JSON 502 satisfies identically to HTML (verified in `tui-door-attach.patch:192-204`). The guard and Part B are **complements, not substitutes**, and Part B is the only thing that stops the frozen TUI. RE-PROMOTED as bead `workstation-fdb1`. The *cost* argument for deferring it (another opencode-patched fork patch, against fable's freeze-the-surface advice, plus a `patched.N` cut and a pool restart) is still real and unchallenged — so this may still be deferred, but it must be deferred **deliberately and with the correct reason**, not on the false equivalence above. **Decide before `mlve.11` starts**, because D4 deploys are exactly the skew-window generators that trigger this freeze.
