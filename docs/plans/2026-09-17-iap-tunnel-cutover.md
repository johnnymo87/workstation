...
**Status:** DONE except the `darwin-rebuild switch`, which needs a sudo password
a human must type. Everything else is implemented and verified live.
...
**Correction to the design below, found during implementation:**
`--listen-on-stdin` is absent from `gcloud compute start-iap-tunnel --help` on
SDK 537.0.0. It is a *hidden* flag, not a missing one -- it is what `gcloud
compute ssh --tunnel-through-iap` uses internally, and it was verified working
as a ProxyCommand here. If a future gcloud drops it, the fallback is
`--local-host-port=localhost:0` plus `nc`, which races on startup.

Also corrected: the script's own IP-rediscovery fallback anchored on
`/Host cloudbox$/`, which stops matching once the block is keyed
`Host cloudbox <IP>`. Left alone, that would have silently disabled IP
rediscovery exactly when user gcloud auth had expired and it was the only
mechanism left. Now `/^Host cloudbox( |$)/`.
...
## Verified after implementation (2026-09-17)

- persistent tunnel's sshd peers are `35.235.243.208` / `.210`, inside
  `35.235.240.0/20` -- it is genuinely on IAP, not the direct path
- `curl http://127.0.0.1:9223/json/version` from cloudbox returns the isolated
  Chrome's UUID `1ed522e8-3fed-416b-bb61-cea1b27a19b3`
- clipboard round-trip through gclpr 2850 succeeds, before and after the
  firewall rule deletion; 2850 listens on loopback only
- `mosh` via the wrapper bootstraps with `SSH_CONNECTION` source `35.235.243.210`
- per-connection cost of the ProxyCommand: direct 0.32s vs IAP 1.74s (best of 3)
- `dev-ssh-warp` deleted. Before deleting, checked that `default-allow-ssh`
  (`0.0.0.0/0` tcp:22) is **disabled**, so it was never the reason anything
  worked; IAP is allowed by two rules (`allow-iap-ssh`, `dev-ssh-iap`) and
  `dev-ssh-client` remains for the WARP-off home-ISP path.
- `nix flake check` fails only on `packages.aarch64-darwin.reset-workspace`
  (systemd on darwin), which reproduces identically at `origin/main`.

## Still to do

- `sudo darwin-rebuild switch --flake .#Y0FMQX93RR-2` -- needed for the mosh
  alias and the hardened tunnel loop. The IAP transport itself is already live
  because `~/.ssh/config` is generated by a script, not by Nix.
- key rotation before 2026-12-16 -- tracked as bead `workstation-h69i`,
  deferred to 2026-12-09.
