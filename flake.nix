  ...
  in {
    ...
    checks.${devboxSystem} = {
      ...
      pressure-sampler-tests = devboxPkgs.runCommand "pressure-sampler-tests" {
        ...
        grep -q '^ALL PASS (27 assertions)' "$TMPDIR/ps.txt" || {
          echo "GATE FAILURE: pressure-sampler suite did not reach ALL PASS (27 assertions)." >&2
      ...
      # suite's keep-assertions were each confirmed to FAIL under a mutation
      # that disables the guard they cover, so a guard that rots cannot stay
      # green.
      disk-cleanup-docker-tests = devboxPkgs.runCommand "disk-cleanup-docker-tests" {
        ...
        grep -q '^=== 30 passed, 0 failed ===' "$TMPDIR/dcd.txt" || {
          echo "GATE FAILURE: disk-cleanup docker suite did not reach its 30/0 tally." >&2
          exit 1
        }
        ...
        [ "$(grep -c '^PASS  ' "$TMPDIR/dcd.txt")" = 30 ] || {
          echo "GATE FAILURE: expected 30 'PASS' lines, got" \
      ...
      oc-cost-tests = devboxPkgs.runCommand "oc-cost-tests" {
        ...
        grep -q '^Ran 104 tests' "$TMPDIR/out.txt" || {
          echo "GATE FAILURE: expected 'Ran 104 tests'. If you added or removed" >&2
      ...
      oc-tags-tests = devboxPkgs.runCommand "oc-tags-tests" {
        nativeBuildInputs = [
          ...
          (devboxPkgs.python3.withPackages (ps: [ ps.tzdata ]))
        ];
      } ''
        cd ${self}
        export HOME="$TMPDIR"
        ...
        # The count is PINNED, following checks.oc-cost-tests. "OK" alone is
        # also what a suite that silently stopped collecting tests prints.
        grep -q '^Ran 185 tests' "$TMPDIR/out.txt" || {
          echo "GATE FAILURE: expected 'Ran 185 tests'. If you added or removed" >&2
          echo "tests deliberately, update the count here in the same commit." >&2
          exit 1
        }
        grep -q '^OK$' "$TMPDIR/out.txt" || {
