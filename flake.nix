  ...
  in {
    ...
    checks.${devboxSystem} = {
      ...
      oc-tags-tests = devboxPkgs.runCommand "oc-tags-tests" {
        ...
        # also what a suite that silently stopped collecting tests prints.
        grep -q '^Ran 159 tests' "$TMPDIR/out.txt" || {
          echo "GATE FAILURE: expected 'Ran 159 tests'. If you added or removed" >&2
          echo "tests deliberately, update the count here in the same commit." >&2
          exit 1
        }
        grep -q '^OK$' "$TMPDIR/out.txt" || {
      ...
      nvim-lua = devboxPkgs.runCommand "nvim-lua-tests" {
        ...
        grep -q '^PASS  session_switcher\.spec unit tests (513 assertions via nvim -l)' "$TMPDIR/out.txt" || {
          echo "GATE FAILURE: session_switcher.spec did not report expected 513 assertions." >&2
          exit 1
        }
        touch $out
      '';
