...
# E) Session-scoped permission/question/MCP routes (patch-only)
...
if [ -n "$LIVE_SID" ]; then
  ...
  assert_route "session-mcp"    "GET"   "/session/${LIVE_SID}/mcp"          "NON-DENY" "ANY"
else
  ...
  printf "[SKIP] %-15s %-6s %-38s | %s\n" "session-mcp"    "GET" "/session/<SID>/mcp"       "No live session available on door"
fi

# Note: POST /session/{sessionID}/questions/{questionID}/reply and /reject, and POST /session/{sessionID}/mcp/{name}/connect and /disconnect,
# are omitted from live gate probes to prevent state mutation / dependency on live pending state; verified deterministically in dispatch.test.ts.
