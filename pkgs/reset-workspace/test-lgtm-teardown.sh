...
grep -q "kill-session -t '=lgtm'" <<<"$teardown" || { echo "FAIL: extracted teardown lacks the kill-session"; exit 1; }
grep -q 'kill -TERM' <<<"$teardown"              || { echo "FAIL: extracted teardown lacks the serialized drain"; exit 1; }
grep -q 'sock_reaped' <<<"$teardown"             && { echo "FAIL: extraction reached the real-/tmp socket reap"; exit 1; }
...
grep -q 'nvim_exited' <<<"$teardown"             && { echo "FAIL: extraction reached shada_watch_report (needs the walk's counter)"; exit 1; }
