...
let
  ...
  opencode-platforms = {
    aarch64-linux = {
      ...
      hash = "sha256-4PbonhXDdimpVaQyV8op5Lh3DaSaHTE8twBlpyw7ziY=";
      isZip = false;
    };
    aarch64-darwin = {
      asset = "opencode-darwin-arm64.zip";
      hash = "sha256-3k7/OAWp92jAakt0tVzVCwzRaniELM61ePn3MPQT9GY=";
      isZip = true;
    };
    x86_64-linux = {
      asset = "opencode-linux-x64.tar.gz";
      hash = "sha256-J5qBD82oLeE/AQH3gg7AXGxag4OLrzCaVMhD1UK8HKA=";
      isZip = false;
    };
    x86_64-darwin = {
      asset = "opencode-darwin-x64.zip";
      hash = "sha256-EJQJWuPvu3aB2GIF3VcHAbMRIB68dQZJG0N8lDd13zk=";
      isZip = true;
    };
  };
  ...
  opencode = let
    ...
    # Adds THREE patches (removing tui-follow-owner, superseded), taking the set
    # to 21, so the interactive attach TUI rides the opaque FRONT DOOR on a
    # SESSION-SCOPED /event?session_ids= stream instead of the /global/event
    # firehose: session-door-routes (server ?session_ids= query field +
    # session-scoped GET pending permissions/questions + POST questions reply/
    # reject + permissionRespond message parity + regenerated SDK), tui-door-attach
    # (scoped subscribe; drop pigeon /route self-resolve — the door owns ownership
    # and drops the SSE leg on migration; D2 child-poll bounded to newest-30 <
    # door's 32-cap + fetch-on-reconnect reconcile; D6 route-follow; NEW-A min-open
    # backoff + jitter; W5 session-scoped reply migration), tui-door-tests (NEW-G
    # client contract + CI step). This bundles Phase 9: oc-pool-attach /
    # oc-auto-attach now `attach $FRONTDOOR_URL` (:4700) instead of the resolved
    # serve, so the door handles owner migration (the removed tui-follow-owner's
    # job). REVIEWED by adversarial-reviewer-fable on the implementation; CRITICAL/
    # HIGH/MEDIUM findings fixed before this cut (see opencode-patched plan doc).
    # DEPLOY ORDER: serves + attach TUIs must restart onto patched.2 together (a
    # workspace reset does this atomically) — an OLD binary through the door 410s
    # on /global/event, and the door's new /session routes 404 on an old serve.
    # (patched.1 was cut then its pin REVERTED after the fable review found the
    # deploy-window regression; patched.2 folds in the fixes + the Phase 9 co-land.)
    #
...
in
{
  ...
  home.activation.generateBazelrc = lib.mkIf (isDarwin || isCloudbox) (lib.hm.dag.entryAfter [ "writeBoundary" ] (let
    bazelrcText = lib.concatStringsSep "\n" ([
      ...
      ""
      "# Repository cache: shared by every workspace and worktree on this host."
      "# Naturally bounded (a few GB of fetched archives) and expensive to refill --"
      "# cloudbox's nightly purge of it refetches 1.4-4 GB of Maven jars every"
      "# morning. Kept on both hosts; only the DISK cache differs, below."
      "common --repository_cache ~/bazel-cache/repository"
      ""
    ] ++ lib.optionals isCloudbox [
      "# NO LOCAL DISK CACHE ON THIS HOST."
      "#"
      "# The empty value is how Bazel 8.5.1 spells \"off\": the disk-cache factory"
      "# enables one only when the value is non-null AND non-empty, and the GC"
      "# idle-task constructor returns null on an empty value, so no cache"
      "# directory is created and no GC is scheduled. Verified empirically in a"
      "# throwaway workspace -- build/query/cquery/info all fine, no warning from"
      "# a leftover --experimental_disk_cache_gc_max_size."
      "#"
      "# Bazel rc precedence is what makes this stick, and it is the same"
      "# precedence the old `--disk_cache ~/bazel-diskcache` line here relied on:"
      "# the HOME rc is read AFTER the workspace rc (verified with"
      "# `bazel info --announce_rc`), so this overrides mono/.bazelrc's"
      "# `common --disk_cache=bazel-cache/diskcache/`."
      "#"
      "# WHY, in the order the evidence supports:"
      "#"
      "# 1. IT DUPLICATES A CACHE WE ALREADY HAVE. mono/.bazelrc owns"
      "#    `common --remote_cache=grpcs://remote.buildbuddy.io`, and since #530"
      "#    nothing here overrides it. In Bazel 8.5.1 --disk_cache is a strict L1"
      "#    in front of that remote L2: CombinedCache checks disk first, writes"
      "#    remote AC hits and CAS blobs BACK into the disk cache, and fans local"
      "#    results out to both. Its only value is latency, and BuildBuddy is"
      "#    close enough from this host for that to be a fair trade: across the"
      "#    344 invocations that moved >10MB (BuildBuddy's own per-invocation"
      "#    cacheStats, 2026-08-10..09-16) download throughput ran p10 2.1 MB/s,"
      "#    median 9.9, p90 23.3. Do not quote a hand-measured RTT here -- TCP"
      "#    connect to remote.buildbuddy.io from cloudbox lands on a GCP load"
      "#    balancer (34.98.106.0) at 1-12ms, while the same probe from a"
      "#    different resolution path reached 216.226.68.241 at ~70ms. The"
      "#    end-to-end throughput above is the number that survived review."
      "#"
      "# 2. IT IS A POOR L1 ANYWAY under mono's --remote_download_outputs=minimal:"
      "#    many output blobs are deliberately never downloaded, so the disk AC"
      "#    entry fails its integrity check and falls through to remote regardless."
      "#"
      "# 3. ITS SIZE BOUND IS A GC TARGET, NOT A QUOTA."
      "#    --experimental_disk_cache_gc_max_size is enforced by an idle task gated"
      "#    on --experimental_disk_cache_gc_idle_delay (5m of SERVER idleness). It"
      "#    never runs during a build, and its cross-process lock is an exclusive"
      "#    tryGet whose loser skips with an INFO log. ON AN ORDINARY DAY THAT IS"
      "#    FINE -- measured 5.0G at the 2026-09-16 nightly and 7.6G across sixteen"
      "#    daytime hours, so the GC does keep up between agent turns. It is NOT"
      "#    fine after a mid-day purge: the 2026-09-15 19:25 manual cleanup reaped"
      "#    23 output bases and the cold rebuild took ~/bazel-diskcache from 0 to"
      "#    24.8G in 65 minutes. Removing the cache removes that excursion class."
      "#"
      "# 4. IT IS WHERE THE OCI IMAGE TARBALLS LAND. mono's tools/deploy.bzl tags"
      "#    oci_load `no-remote-cache`, deliberately excluding those 275-358MB"
      "#    tarballs from BuildBuddy -- but Bazel's disk cache honors only"
      "#    `no-cache`, so they silently accumulated HERE instead. Dropping the"
      "#    disk cache evicts them at ZERO added BuildBuddy transfer, because they"
      "#    were never eligible for BuildBuddy in the first place."
      "#"
      "# WHAT THIS COSTS, so the next BuildBuddy outage does not silently revert"
      "# it: with a WARM disk cache, a fully-cached rebuild used to survive an"
      "# unreachable BuildBuddy -- the capabilities query is lazy and only fires"
      "# when an action must actually execute or upload. Without the L1 the first"
      "# action fails with `Failed to query remote execution capabilities`. The"
      "# practical delta is small because the nightly purge empties the disk cache"
      "# anyway, so every morning's first build was already remote-only. If"
      "# BuildBuddy is down and you need to work, both flags are required --"
      "# `--remote_cache=` alone errors out because mono sets a gRPC downloader:"
      "#     bazel build --remote_cache= --experimental_remote_downloader= //...:"
      "#"
      "# WHAT THIS DOES NOT FIX: per-workspace output bases, the dominant Bazel"
      "# storage class here by an order of magnitude -- 84.2 GB reaped by the"
      "# 2026-09-16 nightly against 5.0 GB of disk cache. That 84.2 GB accrued"
      "# WITH Builds-without-the-Bytes already in force, so do not expect remote"
      "# caching to shrink it. Output-base lifecycle is workstation-o5s1.9."
      "common --disk_cache="
      ""
      "# Do not build non-test targets that a wildcard `test` merely swept up."
      "#"
      "# `bazel test //some/dir/...` expands `...` to EVERY target in the tree and"
      "# builds all of them, tests or not. Under wonder/blueapron/fulfillment that"
      "# pulls in 12 kt_image packages; measured in two live output bases, their"
      "# tarball.tar files alone are 3.88 GB, and counting the server.jar and"
      "# pkg_tar that exist only to feed them the tree total is 7.4 GB. Exactly TWO"
      "# of the 12 images are consumed by anything -- service and repeater, each"
      "# via an ImageTest `data = [\":image-docker\"]` dep -- and those survive this"
      "# flag, because it keeps tests AND their dependencies. The other 10 stop"
      "# being built: 5.4 GB measured directly in one base (which had 9 of the 10"
      "# present), extrapolating to ~6 GB when all 12 packages are built. Output"
      "# bases are the dominant storage class here -- 84.2 GB reaped by one nightly."
      "#"
      "# CAVEAT, because \"tests and their dependencies\" is not the whole rule:"
      "# a test EXCLUDED by --test_tag_filters is not built either, and neither are"
      "# its deps. mono sets `-nocoverage` on coverage and `-noci` on test:ci, and"
      "# ImageTest carries both tags, so under `--config=ci` or `bazel coverage`"
      "# ALL 12 images drop, not 10 -- and noci/nocoverage tests stop being"
      "# COMPILED by a wildcard, which they were before. The agent default"
      "# `--config ai` sets no test tag filter, so the plain path is the 10 above."
      "#"
      "# WHY THIS IS ACCEPTABLE HERE AND WOULD NOT BE ON A SHARED DEV BOX. The"
      "# generic objection is that someone uses a wildcard `test` as a cheap"
      "# compile check over non-test targets. On cloudbox there is no such someone:"
      "# the users are one human and a swarm of agents that treat the test suite as"
      "# the gate and never boot a binary to look at it. CI still runs"
      "# `bazel coverage --config=ci -- //...` (mono .github/workflows/ci.yml:66),"
      "# which builds every non-test target, so a genuinely broken one goes red"
      "# there. The cost is a CI round trip, not an escape to production."
      "#"
      "# THE FOOTGUN THAT SURVIVES THAT ARGUMENT, and the reason this comment is"
      "# long: --build_tests_only also silently drops non-test targets you name"
      "# EXPLICITLY. `bazel test --build_tests_only //foo:some_binary //foo:SomeTest`"
      "# skips some_binary, prints no warning, and exits 0. The hazard is not a"
      "# human missing something -- it is an AGENT concluding it verified a target"
      "# it never built. If you are deliberately naming a non-test target, use"
      "# `bazel build` for it, or pass `--build_tests_only=false`."
      "#"
      "# The real fix is upstream and we have standing to make it: tag what"
      "# kt_image emits in mono's tools/deploy.bzl with `manual`, Bazel's own"
      "# \"exclude me from wildcard expansion\" tag, which mono's scripts/ci.sh"
      "# already honours (:97,:120) and which deploy pipelines are immune to"
      "# because they name `:image-docker` explicitly. Note it must cover more"
      "# than the `%s-docker` filegroup: tagging only that removes the tarball"
      "# output-group request, while `:image`, `:image-tar` and `:image-docker-oci`"
      "# stay in the wildcard, which is roughly half the bytes. That fix would"
      "# cover CI and every developer; this flag only covers this host, and can be"
      "# deleted if it lands. Tracked on workstation-o5s1.9."
      "test --build_tests_only"
    ] ++ lib.optionals isDarwin [
      "# macOS KEEPS its local disk cache. DO NOT UNIFY THIS WITH CLOUDBOX."
      "#"
      "# Every argument for `--disk_cache=` on cloudbox is false on a laptop:"
      "#   - one developer with a handful of worktrees, not 15-100 agent sessions;"
      "#   - no nightly purge and no mid-day cleanup, so the 0->24.8G post-purge"
      "#     excursion that motivates dropping it there cannot occur here;"
      "#   - the hop to BuildBuddy is a laptop's, not a 4ms datacentre hop;"
      "#   - and this machine is sometimes OFFLINE, where the L1 is the only cache"
      "#     there is."
      "#"
      "# The 5G GC target overrides mono/.bazelrc's 10G so the shared cache"
      "# self-limits (2026-08-28 disk-full incident)."
      ...
      "# KotlinKapt (annotation processing, e.g. dagger) is the same ~700MB"
      "# worker class as KotlinCompile; mnemonic verified against rules_kotlin"
      "# 2.3.x compile.bzl (worker-key-mnemonic = \"KotlinKapt\")."
      ...
      "test  --worker_max_instances=KotlinKapt=1"
      ""
      ...
      "# NOTE: this reads /proc/meminfo (whole-host free memory), NOT the cgroup, so it"
      "# does NOT fire when a serve cgroup starves while the box still has free RAM."
      "# That is our actual failure mode -- see the caps below."
      "startup --shutdown_on_low_sys_mem"
      ""
      "# Keep a build from OOM-killing the opencode server it runs inside."
      "#"
      "# The agent's bash tool spawns builds as children of `opencode serve`, so every"
      "# bazel process is charged to that serve's cgroup (MemoryMax=14G on cloudbox)."
      "# opencode-serve@4098 was OOM-killed FOUR times in ~6h on 2026-08-03/04; a capture"
      "# 35s before the third kill showed 7.61G of the cgroup's 9.04G held by bazel (server"
      "# JVMs plus ~100 concurrent processwrapper-sandbox javac actions at ~0.5G each)."
      "# Because OOMPolicy=stop, ANY OOM kill in that cgroup restarts the whole serve"
      "# and takes every session on it down -- so the build must stay under the ceiling."
      "# (The kernel also picks the largest anon process as victim, which is opencode"
      "# itself, not the many small bazel actions. Victim choice is moot under stop.)"
      "#"
      "# THESE ARE A MITIGATION, NOT THE FIX, and they are kept as DEFENCE IN DEPTH."
      "# --jobs is per-invocation, so two concurrent builds on one serve still breach"
      "# the ceiling, and nothing here bounds the aggregate across invocations, server"
      "# JVMs, or throwaway worktrees."
      "#"
      "# The structural fix ships alongside them: `bazel` on PATH is now a shim"
      "# (pkgs/bazel-scope) that re-execs the build into its own systemd scope under"
      "# bazel.slice, so builds are not charged to the serve cgroup at all. These caps"
      "# still matter, for two reasons: they bound what one build can do INSIDE its"
      "# 10G scope, and they are the only thing standing between a build and the serve"
      "# on the shim's degrade path (when systemd-run is unusable). Do not remove them"
      "# on the grounds that the shim supersedes them."
      "#"
      "# The worker/idle-server limits above do not help here: they bound PERSISTENT"
      "# workers and IDLE servers, neither of which is the sandboxed action fleet."
      "# --jobs is the hard concurrency cap; --local_resources is estimate-based"
      "# scheduling and defaults to HOST_RAM*0.67 (~41G here), i.e. the scheduler"
      "# otherwise believes it has 3x the room the cgroup actually allows."
      "build --jobs=8"
      "build --local_resources=memory=4096"
      ""
      "# --config=remote sends compiles to RBE but keeps TEST RUNNERS local"
      "# (mono/.bazelrc: `build:remote --jobs=50` + `test:remote --strategy=TestRunner=local`),"
      "# so without this a remote test run may start ~50 local test processes in the"
      "# serve cgroup -- the one path where the --jobs cap above is overridden."
      "test --local_test_jobs=4"
      ""
      "# Bound the server JVM heap. Without -Xmx the JVM sizes from the cgroup limit"
      "# (container-aware: 14G/4 = 3.5G max heap each), and several workspaces can have"
      "# a resident server at once -- 2.9G across two was observed idle on one serve."
      "# Startup options only take effect on server restart."
