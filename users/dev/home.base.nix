...
let
  ...
  opencode-platforms = {
    aarch64-linux = {
      ...
      hash = "sha256-h6a91LLofSqZmeHJy3DZMGoCUwThGGYYjCI8yjQyBF8=";
      isZip = false;
    };
    aarch64-darwin = {
      asset = "opencode-darwin-arm64.zip";
      hash = "sha256-RlcST+MfkD1XO+wC8ijLw9e7bxs+dKh6MrQ4fwuXpNs=";
      isZip = true;
    };
    x86_64-linux = {
      asset = "opencode-linux-x64.tar.gz";
      hash = "sha256-udM0sxq32zfLIwOGE76Dq4TQonxzmK4WfW9vjugVIRc=";
      isZip = false;
    };
    x86_64-darwin = {
      asset = "opencode-darwin-x64.zip";
      hash = "sha256-b8fxkEGB9ZxDbaR5H4U+aMPj4QTSuKCAUaMTzXydDeQ=";
      isZip = true;
    };
  };
...
in
{
  ...
  home.activation.generateBazelrc = lib.mkIf (isDarwin || isCloudbox) (lib.hm.dag.entryAfter [ "writeBoundary" ] (let
    staticHeader = lib.concatStringsSep "\n" [
      ...
      ""
      "# Local disk and repository caches. NOTE bazel rc precedence: the HOME"
      "# rc is read AFTER the workspace rc, so this --disk_cache overrides"
      "# mono/.bazelrc's relative path — mono builds write ~/bazel-diskcache."
      "# The 5G GC target likewise overrides mono's 10G: bazel's idle-time GC"
      "# keeps the shared cache self-limiting intraday instead of sitting at"
      "# 10G until the nightly purge (2026-08-28 disk-full incident)."
