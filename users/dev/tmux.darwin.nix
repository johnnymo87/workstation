...
# WARNING: Do NOT enable @continuum-boot on macOS - causes Accessibility prompt issues
  ...
  programs.tmux = {
    ...
    plugins = with pkgs.tmuxPlugins; [
      # 1. Resurrect: save/restore sessions
      {
        plugin = resurrect;
        extraConfig = ''
          # Persist resurrect data in XDG state directory
      ...
      # 3. Continuum: auto-save/restore (MUST BE LAST - uses status-right hook)
      {
        ...
        extraConfig = ''
          ...
          # NOTE: @continuum-boot is NOT enabled - causes macOS Accessibility prompt issues
        '';
      }
    ];
  };
